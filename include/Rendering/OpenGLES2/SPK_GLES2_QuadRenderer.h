...
#ifndef H_SPK_GLES2_GLQUADRENDERER
#define H_SPK_GLES2_GLQUADRENDERER
...
namespace GLES2
{
	...
	* </ul>
	*/
	class SPK_GLES2_PREFIX GLES2QuadRenderer :	public GLES2Renderer,
	...
	public :
		...
		/**
		* @brief Creates and registers a new GLES2QuadRenderer
		* @param scaleX the scale of the width of the quad
		* @param scaleY the scale of the height of the quad
		* @return A new registered GLES2QuadRenderer
		*/
		static  Ref<GLES2QuadRenderer> create(float scaleX = 1.0f,float scaleY = 1.0f);
		...
		/**
		* @brief Gets the texture of this GLES2QuadRenderer
		* @return the texture of this GLES2QuadRenderer
		*/
		GLuint getTexture() const;

	public :
		spark_description(GLES2QuadRenderer, GLES2Renderer)
		(
		);
	...
	private :
		...
		GLES2QuadRenderer(float scaleX = 1.0f,float scaleY = 1.0f);
		GLES2QuadRenderer(const GLES2QuadRenderer& renderer);
		...
		void GLCallColorAndVertex(const Particle& particle,GLES2Buffer& renderBuffer) const;	// OpenGL calls for color and position
		void GLCallTexture2DAtlas(const Particle& particle,GLES2Buffer& renderBuffer) const;	// OpenGL calls for 2D atlastexturing 
		void GLCallTexture3D(const Particle& particle,GLES2Buffer& renderBuffer) const;		// OpenGL calls for 3D texturing

		mutable void (GLES2QuadRenderer::*renderParticle)(const Particle&,GLES2Buffer& renderBuffer) const;	// pointer to the right render method

		void render2D(const Particle& particle,GLES2Buffer& renderBuffer) const;			// Rendering for particles with texture 2D or no texture
		void render2DRot(const Particle& particle,GLES2Buffer& renderBuffer) const;		// Rendering for particles with texture 2D or no texture and rotation
		void render3D(const Particle& particle,GLES2Buffer& renderBuffer) const;			// Rendering for particles with texture 3D
		void render3DRot(const Particle& particle,GLES2Buffer& renderBuffer) const;		// Rendering for particles with texture 3D and rotation
		void render2DAtlas(const Particle& particle,GLES2Buffer& renderBuffer) const;		// Rendering for particles with texture 2D atlas
		void render2DAtlasRot(const Particle& particle,GLES2Buffer& renderBuffer) const;	// Rendering for particles with texture 2D atlas and rotation

		static const char* VERTEX_SOURCE;
		static const char* FRAGMENT_SOURCE;
	};

	inline Ref<GLES2QuadRenderer> GLES2QuadRenderer::create(float scaleX,float scaleY)
	{
		return SPK_NEW(GLES2QuadRenderer,scaleX,scaleY);
	}
		
	inline void GLES2QuadRenderer::setTexture(GLuint textureIndex)
	{
		this->textureIndex = textureIndex;
	}

	inline GLuint GLES2QuadRenderer::getTexture() const
	{
		return textureIndex;
	}

	inline void GLES2QuadRenderer::GLCallColorAndVertex(const Particle& particle,GLES2Buffer& renderBuffer) const
	{
	...
	inline void GLES2QuadRenderer::GLCallTexture2DAtlas(const Particle& particle,GLES2Buffer& renderBuffer) const
	{
	...
	inline void GLES2QuadRenderer::GLCallTexture3D(const Particle& particle,GLES2Buffer& renderBuffer) const
	{
	...
	inline void GLES2QuadRenderer::invertModelView() const
	{
		float tmp[12];
		float src[16];
